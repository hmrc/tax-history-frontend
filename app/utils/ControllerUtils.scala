...
object ControllerUtils {
  ...
  def displayTaxCodeHeading(taxYear: Int, employmentStatus: EmploymentStatus, employmentEndDate: Option[LocalDate])(
    ...
    if (
      TaxYear.current.startYear == taxYear && (employmentStatus == EmploymentStatus.Live || employmentStatus == EmploymentStatus.Unknown || employmentStatus == EmploymentStatus.PotentiallyCeased)
    ) {
